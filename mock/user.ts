...
export default {
  // 支持值为 Object 和 Array
  'GET /console/api/currentUser': (req: Request, res: Response) => {
    if (!getAccess()) {
      res.status(401).send({
        data: {
          isLogin: false,
  ...
  'POST /console/api/login/account': async (req: Request, res: Response) => {
    const { password, username, type } = req.body;
    await waitTime(1000);
    if (password === '321031x' && username === 'admin') {
      res.send({
        status: 'ok',
        type,
  ...
  'POST /console/api/login/outLogin': (req: Request, res: Response) => {
    access = '';
    res.send({ data: {}, success: true });
  },
  'POST /console/api/register': (req: Request, res: Response) => {
    res.send({ status: 'ok', currentAuthority: 'user', success: true });
  },
  'GET /console/api/500': (req: Request, res: Response) => {
    res.status(500).send({
  ...
  'GET /console/api/404': (req: Request, res: Response) => {
    res.status(404).send({
  ...
  'GET /console/api/403': (req: Request, res: Response) => {
    res.status(403).send({
      timestamp: 1513932555104,
      status: 403,
      error: 'Forbidden',
  ...
  'GET /console/api/401': (req: Request, res: Response) => {
    res.status(401).send({
      timestamp: 1513932555104,
      status: 401,
      error: 'Unauthorized',
  ...
  'GET  /console/api/login/captcha': getFakeCaptcha,
};
